package com.example.trackingv1;

import static com.example.trackingv1.MainActivity.JOB_STATE_CHANGED;
import static com.example.trackingv1.MainActivity.LOCATION_ACQUIRED;
...
//                        Toast.makeText(getBaseContext(),"new point",Toast.LENGTH_SHORT).show();
                    ...
                    if(cd.isConnectingToInternet()) { // check whether internet is available or not
                        ...
                        SimpleDateFormat currentDateFormat = new SimpleDateFormat("MM:dd:yyy HH:mm");
                        String currentDate = currentDateFormat.format(calForDate.getTime());
                        System.out.println(currentDate);
                        databaseReference.child("Driver Location").child("driver_id").child("longitude").setValue(location.getLongitude());
                        databaseReference.child("Driver Location").child("driver_id").child("latitude").setValue(location.getLatitude());
                        databaseReference.child("Driver Location").child("driver_id").child("latest update").setValue(currentDate);

                        System.out.println("I'm changed:" +location.getLatitude()+" : "+location.getLongitude());
